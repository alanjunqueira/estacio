  ...
  public void run() {
    try {
      synchronized (lista_threads_id) {
        lista_threads_id = lista_threads_id + "[" + Thread.currentThread().getId() + "]"; // Constrói na instância de
                                                                                          // Empacotador uma lista com
                                                                                          // todas as threads que foram
                                                                                          // criadas.
      }
      System.out.println(nome + " empacotando (" + System.currentTimeMillis() + ")");
      Thread.sleep((int) (Math.random() * 899 + 100)); // Coloca a thread para dormir por um período aleatório entre 100
                                                       // e 999 milissegundos.
