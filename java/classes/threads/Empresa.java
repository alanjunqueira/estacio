...
public class Empresa {
  ...
  private final ArrayList<Equipe> turno; // Conjunto de equipes de empacotadores. Cada equipe possui 2 ou mais
                                         // empacotadores e corresponde a uma thread.
  private final int max_prod_empacotar; // Número máximo de produtos que serão empacotados.
  private final int pool_empacotadores; // Número de empacotadores disponíveis para formar as equipes. Cada empacotador
                                        // também corresponde a uma thread.
  ...
  public Empresa(int nr_fitas, int pool_empacotadores, int nr_max_equipes, int max_prod_empacotar)
    ...
    if ((nr_fitas < 1) || (pool_empacotadores < 2) || (nr_max_equipes < 1) || (max_prod_empacotar < 1))
      ...
      criarEquipes(nr_fitas); // Monta as equipes alocando os empacotadores e armazenando as equipes em
                              // "turno".
      turno.forEach((eqp) -> eqp.start()); // Inicia todas as threads.
      for (Equipe eqp : turno) // Faz o join com todas as threads de equipe.
        try {
          eqp.join(); // A thread principal deve aguardar o fim de todas as threads Equipe para poder
                      // contabilizar os empacotamentos.
  ...
  private void criarEquipes(int nr_fitas) {
    ...
    do { // Cria as equipes com um número aleatório de integrantes e as adiciona em
      ...
      if (nr_emp_eqp > nr_fitas)
        nr_emp_eqp = nr_fitas; // Do contrário, teríamos mais solicitações de acesso do que o semáforo possui e
                               // a thread bloquearia eternamente.
     ...
     * empregados for maior do que o número de fitas,
     * são alocados nr_fitas empregados (caso contrário, a equipe iria solicitar um
     * número de permissões superior ao número
